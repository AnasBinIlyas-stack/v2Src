...
function Dashboardcard({ productId, isListed, imgsrc, detail, price, description, tokenId, type, startCheckout }) {
  ...
  const getApprovedfunc = async () => {
    if (tokenId) {
      try {
        const approved = await landingContract.methods.getApproved(tokenId).call();
        setIsApproved(approved !== "0x0000000000000000000000000000000000000000"); // Check if not zero address
      } catch (error) {
  ...
  const approveToken = async () => {
    ...
    if (tokenId) {
      ...
      let transaction = {
        to: landingAddress,
        chainId: chainId,
        data: landingContract.methods.approve(marketplaceAddress, tokenId).encodeABI(),
      };
  ...
  return (
    <section className="dashboardcard-sec">
      <div className="inner">
        <div className="top">
          <img src={imgsrc} alt="" style={{ objectFit: "cover", borderRadius: "6px" }} />
          <div className="card-layer"></div>
        </div>
        <div className="center">
          <div className="price">
            <p className="text-white text-inter fw-semibold text-truncate" style={{ maxWidth: '60%' }}>{detail}</p>
            {price && <p className="text-white text-inter fw-semibold fs-6">{price} BNB</p>}
          </div>
        ...
        {!isListed && type == "land" ? (
          isApproved ? (
            <button className="bottom text-white textexpansiva-bold" onClick={() => listLandNft(price)}>List for sale</button>
          ) : (
            <button className="bottom text-white textexpansiva-bold" onClick={approveToken}>Approve</button>
          )
        ) : (
          <button
