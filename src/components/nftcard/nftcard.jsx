...
function Nftcard({
  ...
  const buyLandNft = async () => {
    ...
    let transction = {
      to: marketplaceAddress,
      chainId: chainId,
      data: marketplaceContract.methods.buyLandNft(count?.toString(), priceOfNft?.toString()).encodeABI(),
      ...
      <div className={`inner ${type === "sold" ? "blur-card" : ""}`}>
        ...
        {!isListed ? (
          ...
          <div className="text-white text-inter fw-bold">
            <p>List Price: {listedPrice ? `${listedPrice} ETH` : 'Price not available'}</p>
          </div>
        )}
        {type === "sold" && (
          <div className="sold">
